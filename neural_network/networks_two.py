...
def create_networks(public_bits, private_bits, dropout_rate):
    ...
    HOinput_addition =  concatenate([HO_reshape1_addition, HO_reshape2_addition], axis=-1)
    x_a = NAC(units)(HOinput_addition)
    x_a = NAC(1)(x_a)
    x_a = Reshape((c3_bits,))(x_a)

    HO_model_addition = Model(inputs=[HOinput1_addition, HOinput2_addition], outputs=x_a)
    ...
    HOinput_multiplication =  concatenate([HO_reshape1_multiplication, HO_reshape2_multiplication], axis=-1)
    x_m = NALU(units)(HOinput_multiplication)
    x_m = NALU(1)(x_m)
    x_m = Reshape((c3_bits,))(x_m)

    HO_model_multiplication = Model(inputs=[HOinput1_multiplication, HOinput2_multiplication], outputs=x_m)
    ...
    HOout_addition = HO_model_addition([aliceout1, aliceout2])
    HOout_multiplication = HO_model_addition([aliceout1, aliceout2])
    ...
    evemodel = Model([ainput0, ainput1, ainput2, anonce_input], [eveout_addition, eveout_multiplication], name='evemodel')
